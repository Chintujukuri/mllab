// Prt_value scheduling with arrivial time is different for each process:
#include <stdio.h>
#include <stdlib.h>
...
void sort_by_prt_value(process *p, int n)
{
    ...
    for (int i = 0; i < n - 1; i++)
    {
        ...
        {
            if (p[j].prt_value > p[j + 1].prt_value)
                swap(&p[j], &p[j + 1]);
        }
    }
}
...
void prt_valueScheduling(process *p, int n)
{
    ...
    // rt is an array to store the remaining time for each process. It is initialized to burst time of each process.
    for (int i = 0; i < n; i++)
        rt[i] = p[i].bt;
